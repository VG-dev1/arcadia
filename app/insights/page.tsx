...
export function InsightsPageContent() {
  ...
  const metrics = useMemo(() => {
    ...
    monthlyTasks.forEach(task => {
      ...
      grandTotalOverTime += task.overTime;
      overTimeTasksCount += 1;
    ...
    if (grandTotalOverTime === 0) {
      overTimeMessage = "Perfect month so far — you've stayed on schedule for every task.";
    } else if (grandTotalOverTime <= 15) {
      overTimeMessage = "Just a few minutes behind overall. You're staying very consistent.";
    } else if (grandTotalOverTime <= 60) {
      overTimeMessage = "A small amount of overtime has added up. There's room for a few quick wins.";
    } else if (grandTotalOverTime <= 180) {
      overTimeMessage = "You've spent noticeable extra time on tasks this month. Consider leaving a little more buffer in your schedule.";
    } else if (grandTotalOverTime <= 360) {
      overTimeMessage = "Overtime is becoming a pattern. Reviewing task durations could make your days feel less rushed.";
    } else if (grandTotalOverTime <= 720) {
      overTimeMessage = "You've accumulated many extra hours this month. Try prioritizing the most important tasks and reducing overcommitment.";
    } else {
      overTimeMessage = "This month has involved significant overtime. Take a step back and see whether your schedule is asking too much of you.";
    }
      ...
      <div style={{ padding: "40px 24px", maxWidth: "1200px", margin: "0 auto" }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <h2 style={{ fontSize: "24px", margin: 0 }}>Total minutes of finishing tasks late</h2>
            <h2 style={{ fontSize: "56px", fontWeight: "bold", margin: 0, color: "#fff" }}>
              {metrics.grandTotalOverTime}
            </h2>
            <div style={{ padding: "0 10px" }}>
              <p style={{ fontSize: "14px", color: "#aaa", lineHeight: "1.5", margin: 0 }}>
                {metrics.overTimeMessage}
              </p>
            </div>
          </div>
